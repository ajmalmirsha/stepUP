...
async function addUser(userData, req, res) {
...
module.exports = {
  ...
  sendOTP: async (req, res, next) => {
    try {
      if (
        ...
        const exist = await user.find({ email: userEmail });
        ...
        if (!!exist.length) {
          ...
          if (!!number) {
            ...
            res.render("usersignup", {
              err: "this Phone number  already exist !!",
            });
          } else {
            console.log("on else");
            ...
            try {
              await addUser(req.body, req, res)
  ...
  view_detail: async (req, res, next) => {
    try {
      const singleproduct = await product
        .find({ _id: req.params.id, unlisted: false })
        .populate("catagory")
        .populate("brand")
        .exec();
      const categoryData = await catagory.find();
      if (req.session.user) {
        res.render("products/view-detail", {
          singleproduct,
          categoryData,
          users: true,
        });
  ...
  catagory: async (req, res, next) => {
    try {
      const data = await product
        .find({ catagory: req.params.id, unlisted: false })
        .populate("catagory")
        .exec();
      if (data) {
  ...
  lowtohigh: async (req, res, next) => {
    try {
      if (
        ...
        const data = await product
          .find({ catagory: req.query.cataId })
          .sort({ price: 1 });
        const cata = await catagory.find();
        res.render("shop", { data, cata, sort: 1, cataId: req.query.cataId });
      } else if (
        typeof req.query.cataId !== undefined &&
        typeof req.query.search !== undefined
      ) {
        const search = req.query.search;
        const ss = new RegExp(search, "i");
        const data = await product
          .find({ productname: ss, catagory: req.query.cataId })
          .sort({ price: 1 });
        const cata = await catagory.find();
        res.render("shop", {
          data,
        ...
        .then(async (verificationResponse) => {
          if (verificationResponse.status === "approved") {
            ...
            res.render("forgot/forgot_otp", {
              err: "wrong otp you have entered",
            });
          }
        });
    } catch (error) {
      ...
      if (old == "" || newPass == "") {
        ...
        await bcrypt.compare(old, exist.password).then(async (result) => {
          if (result) {
            ...
            res.render("profile/user-profile", {
              data,
              err: "you entered a wrong password !!",
            });
          }
        });
      }
  ...
  edit_address: async (req, res, next) => {
    try {
      if (
        ...
        await user.updateOne(
          { email: req.session.email, "address._id": req.params.id },
          {
            $set: {
              "address.$": req.body,
            },
        ...
        for (var i = 0; i < data.cart.length; i++) {
          if (data.cart[i].productId.quantity <= 0) {
            ...
            res.render("cart/cart", {
              cartData,
              message: "remove outof stock item from the cart",
              users: true,
            });
          } else {
            rr = rr + 1;
        ...
        if (couponUsed) {
          ...
          if (coupons.date < Date.now()) {
            ...
            const data = await user
              ...
              .exec();
            const per =
              (data.cartTotalPrice * coupons.discountpercentage) / 100;
            if (per > coupons.maxdiscountprice) {
              const tprice = data.cartTotalPrice - coupons.maxdiscountprice;
              res.json({
                success: true,
                message:
                  "the coupon limit price is :" + coupons.maxdiscountprice,
                coupondata: coupons,
                user: data,
                tprice,
              });
  ...
  after_checkout: async (req, res, next) => {
    try {
      ...
      async function emptycart() {
        await user.updateOne(
          { email: req.session.email },
          { $pull: { cart: { productId: { $in: products.productId } } } }
        );
        await user.updateOne(
          { email: req.session.email },
      ...
      if (products.paymentMethod == "COD") {
        ...
        await couponModel.updateOne(
          { code: code },
          { $addToSet: { used: req.session.email } }
        );
  ...
  order_cancel: async (req, res, next) => {
    try {
      ...
      await orderModel
        .updateOne({ orderId: st }, { $set: { status: req.params.status } })
        .then(() => {
